...
# H2, O2, N2, F2, Cl2, NaCl  # diatomic molecule
...
def start(input_element,element_num=2):
    ...
    if output:
        for i in range(len(output)):
            show.append(('와 '.join(output[i]),'분자식 : '+''.join(molecular_formula[i])))
    else:
        show.append('결합 가능한 분자식이 없습니다 ')
    return show
# print(start(["H","O"]))
